...
print_names = c("Independent Parallel", "Parallel two-stage negative-first","Parallel two-stage balanced (wait)", "Parallel two-stage balanced", "Sequential Enrichment")

df = df[df$Design %in% the_design_names, ]

# colours
cols = brewer.pal(7, "Accent")[c(1,3,7,5,6)]
...
text(0.5,length(the_design_names):1, labels = print_names, pos = 4, cex = 0.95)
