...
outcome = simulateOutcome(fun = function(data, decision, ...){
  ...
  if(decision@stage == "1" | (decision@stage == "2" & data$biomarker[rw] > 0)){
    if(p$stopRule == "pause" & decision@stage == "1" & decision@part == "a"){
        if((data$biomarker[rw] == 0 & nPat0<p$nInt[1]) | (data$biomarker[rw] == 1 & nPat1<p$nInt[2])){
    ...
    } else if(p$stopRule == "pause" & decision@stage == "1" & decision@part == "b"){
...
triggerAnalysis = readyForAnalysis(fun = function(data, decision, ...){
  ...
  } else if(p$stopRule %in% c("smallest","pause")){
    if(decision@stage == "1") {
      if(decision@part == "a") {
...
theDecision = makeDecisions(fun = function(data, model, ...) {
  ...
  if(p$decision@stage == "1" & p$decision@part == "a"){
    # first interim
    # test b- and b+
    if(postProb(model$postAll,p$biomarkerH1.0, 0) < p$postProbIAll | postProb(model$postAll,p$biomarkerH0.0, 1) > p$postProb){
      if((nPat0 < p$nFinal[1] & postProb(model$post0,p$biomarkerH1.0, 0) < p$postProbIF) | (nPat0 >= p$nFinal[1] & postProb(model$post0,p$biomarkerH0.0, 1) > p$postProb)){
        stage = "1"
        part = "b"
        trail = c(trail, paste0(stage,part))
      } else {
  ...
  } else if(p$decision@stage == "1" & p$decision@part == "b"){
    # test b- and b+
  ...
  } else if(p$decision@stage == "2" & p$decision@part == "b"){
    if(postProb(model$post1,p$biomarkerH0.1, 1) > p$postProb){
      continue = FALSE
...
setup = splitDataDesign(
  ...
  p = list( # options to adjust trial
    ...
    stopRule = c("total","totalSmall","smallest","pause")[4],
