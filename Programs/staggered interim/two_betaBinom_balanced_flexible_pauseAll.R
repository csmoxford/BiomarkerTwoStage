  ...
  if(testAll) {
    ...
    if(nPat0 >= p$nInt[1]) {
      ...
      if(testNegative) {
        ...
        # Evidence of futility in negative group
        recruit = "+ve"
        staging0 = "."
...
if(run){
  ...
  nrecruited = sapply(res@sims, function(x) return(sum(!is.na(x$data$outcomeTime))))
  max(nrecruited)
  ...
  table( res@sims[[i]]$data$biomarker,res@sims[[i]]$data$cohort,res@sims[[i]]$data$tested, useNA = "ifany")
