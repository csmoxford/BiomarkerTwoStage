...
source(paste0(programDir,"run_table1.R"), echo = TRUE)

# Note that the Enrichment and independent designs are not simulated but have their own code to generate the table:
# source(paste0(programDir,"run_table1_Zang_Yuan.R"), echo = TRUE)
# source(paste0(programDir,"run_table1_Independent_parallel.R"), echo = TRUE)

# Compile summary data from simulations
source(paste0(programDir,"getData_table1.R"), echo = TRUE)

# Compile table 1 to a word document
render(
  input = paste0(programDir,"getTable_table1.Rmd"),
  output_file = paste0(programDir,"Table 1.docx")
)

###############################################################
## Run the simulations for figure 6 of the paper
source(paste0(programDir,"run_figure5.R"), echo = TRUE)

# Compile summary data and produce figure 6
source(paste0(programDir,"getFigure_figure5.R"), echo = TRUE)

###############################################################
## Run the simulations for figure 8 of the paper
source(paste0(programDir,"run_figure6.R"), echo = TRUE)

# Compile summary data and produce figure 8
source(paste0(programDir,"getFigure_figure6.R"), echo = TRUE)
